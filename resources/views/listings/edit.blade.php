...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <h4 class="fw-bold mb-4" style="color:var(--lokabuy-primary)">Edit Ad</h4>


            <form method="POST" action="{{ route('listings.update', $listing->slug) }}"
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3" x-show="subcategories.length > 0" x-cloak>
                            ...
                            <select name="subcategory_id"
                                ...
                                <template x-for="sub in subcategories" :key="sub.id">
                                    <option :value="sub.id"
                                            :selected="sub.id === {{ $listing->subcategory_id ?? 'null' }}"
                        ...
                        <div class="mb-3" x-show="isRealEstate" x-cloak>
                            ...
                            <select name="listing_type" class="form-select">
                                ...
                                <option value="buy"  {{ old('listing_type', $listing->listing_type) == 'buy'  ? 'selected' : '' }}>For Sale</option>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" inputmode="decimal" name="price"
                                   ...
                                   value="{{ old('price', $listing->getRawOriginal('price')) }}" required
                                   autocomplete="off">
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="card border-0 shadow-sm rounded-3 mb-4">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-baseline mb-1"
                             x-data="{ get count() { return {{ $listing->images->count() }}; } }">
                            <h6 class="fw-bold mb-0">Photos</h6>
                            <span class="small text-muted">{{ $listing->images->count() }} / 10</span>
                        </div>
                        ...
                        <div class="d-flex flex-wrap gap-2 mb-3"
                             x-data="existingImages({{ $listing->images->toJson() }}, '{{ $listing->slug }}')">
                            <template x-for="img in images" :key="img.id">
                                <div class="position-relative">
                                    <img :src="img.url" class="rounded" width="90" height="72"
                        ...
                        <div x-data="imagePicker()">
                            <label class="d-block border border-2 border-dashed rounded-3 text-center py-3 px-3"
                                   style="cursor:pointer;border-color:var(--lokabuy-border)!important"
                                   for="imageInput">
                                <i class="bi bi-plus-circle fs-4 d-block mb-1" style="color:var(--lokabuy-primary)"></i>
                                <span class="small fw-semibold" style="color:var(--lokabuy-primary)">Add more photos</span>
                            ...
                            <div class="d-flex flex-wrap gap-2 mt-2" x-show="previews.length > 0" x-cloak>
                        ...
                        @else
                        <p class="text-muted small mb-0">Maximum of 10 images reached. Remove one to add more.</p>
                        @endif
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="location_city"
                                       ...
                                       value="{{ old('location_city', $listing->location_city) }}" required>
                                @error('location_city')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="text" name="location_state"
                                       ...
                                       value="{{ old('location_state', $listing->location_state) }}" required>
                            ...
                            <div class="col-md-6">
                                ...
                                @include('partials._country-select', ['currentValue' => $listing->location_country ?? 'India'])
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('listings.my') }}" class="btn btn-outline-secondary px-4 py-2">
                        Cancel
                    </a>
                </div>
...
function listingForm() {
    return {
        ...
        subcategories: allSubcategories['{{ old("category_id", $listing->category_id) }}'] ?? [],
        isRealEstate: '{{ $listing->category->slug ?? "" }}' === realEstateSlug,
...
function existingImages(initialImages, slug) {
    return {
        ...
        async remove(id) {
            ...
            try {
                ...
                await fetch(`/listings/${slug}/images/${id}`, { method: 'POST', body });
